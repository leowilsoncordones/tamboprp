...
namespace Datos
{
    public class Control_ProduccMapper: AbstractMapper
    {
        ...
        protected Control_Producc load(SqlDataReader record)
        {
            ...
            cp.Grasa = (float)((DBNull.Value == record["GRASA"]) ? 0 : (float)record["GRASA"]);
            cp.Leche = (DBNull.Value == record["LECHE"]) ? 0 : (float)record["LECHE"];
            ...
            cp.Dias_para_control = (int)((DBNull.Value == record["DIAS_DEL_MES"]) ? 0 : (int)record["DIAS_DEL_MES"]);
